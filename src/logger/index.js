...
function createDefaultConfig() {
  return {
    appName: process.env.LOG_APP_NAME || 'marksimos',
...
function createRequestLogger(logger, options) {
  var requestLogger = logger || createLogger('http');
...
function writeToFile(config, line) {
  ...
  try {
    ...
    console.error('[marksimos-share/logger] failed to write log file: %s', err && err.message ? err.message : err);
  }
}
